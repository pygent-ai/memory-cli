...
export function memoryProjectExists() {
...
export function loadMemories() {
  ...
  return fs.readdirSync(memoryDir)
    ...
    .map((name) => {
      ...
      return { ...readJson(filePath), _path: filePath };
    });
}
...
export function showMemory(memoryId) {
...
export function addMemory(candidate, force = false) {
...
export function updateMemory(memoryId, updates) {
  ...
  const memory = { ...readJson(target), ...updates, id: memoryId, updated_at: new Date().toISOString().slice(0, 10) };
  writeJson(target, memory);
  return { status: "updated", id: memoryId, path: target };
}

export function retireMemory(memoryId, reason) {
  const updates = { status: "retired", retired_at: new Date().toISOString().slice(0, 10) };
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (command === "init") result = initProject(valueAfter(args, "--path") ?? ".");
    ...
    if (["conflict", "invalid", "exists"].includes(result.status)) code = 1;
  } else if (command === "list") result = listMemories();
  else if (command === "show") {
    result = showMemory(args[0]);
    if (result.error) code = 1;
  } else if (command === "update") {
    result = updateMemory(args[0], readJson(requiredValue(args, "--file")));
    if (result.status === "not_found") code = 1;
  } else if (command === "retire") {
    result = retireMemory(args[0], valueAfter(args, "--reason"));
    if (result.status === "not_found") code = 1;
  } else if (command === "test") {
    result = runTests();
    if (result.blocking_failed) code = 1;
